...
const encoding13 = {
  preencode(state, m) {
    ...
    if (version >= 2 && m.modelId) c.string.preencode(state, m.modelId)
    if (version >= 2 && m.modelName) c.string.preencode(state, m.modelName)
    if (version >= 2 && m.loadedAt) c.int.preencode(state, m.loadedAt)
  },
  encode(state, m) {
    const flags =
      ((version >= 2 && m.modelId) ? 1 : 0) |
      ((version >= 2 && m.modelName) ? 2 : 0) |
      ((version >= 2 && m.loadedAt) ? 4 : 0) |
      ((version >= 2 && m.accepting) ? 8 : 0)

    c.buffer.encode(state, m.writerKey)
    c.uint.encode(state, flags)

    if (version >= 2 && m.modelId) c.string.encode(state, m.modelId)
    if (version >= 2 && m.modelName) c.string.encode(state, m.modelName)
    if (version >= 2 && m.loadedAt) c.int.encode(state, m.loadedAt)
  },
  decode(state) {
    const r0 = c.buffer.decode(state)
    const flags = c.uint.decode(state)
    ...
    return {
      writerKey: r0,
      modelId: (version >= 2 && (flags & 1) !== 0) ? c.string.decode(state) : null,
      modelName: (version >= 2 && (flags & 2) !== 0) ? c.string.decode(state) : null,
      loadedAt: (version >= 2 && (flags & 4) !== 0) ? c.int.decode(state) : 0,
      accepting: (version >= 2 && (flags & 8) !== 0)
    }
  }
}
...
const encoding16 = {
  preencode(state, m) {
    ...
    if (version >= 2 && m.text) c.string.preencode(state, m.text)
    if (version >= 2 && m.error) c.json.preencode(state, m.error)
  },
  encode(state, m) {
    const flags = ((version >= 2 && m.text) ? 1 : 0) | ((version >= 2 && m.error) ? 2 : 0)
    ...
    if (version >= 2 && m.text) c.string.encode(state, m.text)
    if (version >= 2 && m.error) c.json.encode(state, m.error)
  },
  decode(state) {
    const r0 = c.string.decode(state)
    const r1 = c.buffer.decode(state)
    ...
    return {
      ...
      toKey: r2,
      kind: r3,
      text: (version >= 2 && (flags & 1) !== 0) ? c.string.decode(state) : null,
      error: (version >= 2 && (flags & 2) !== 0) ? c.json.decode(state) : null
    }
  }
}
